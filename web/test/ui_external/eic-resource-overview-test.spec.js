...
async function selectStandardSizeEICResourceOverview(page){
  ...
  await page.click('text=Please select...');
  await page.click('text=2.19.0-8');
...
const clusterRecommendationTests = [
  {index: 1, name: 'CPU Recommendation', expectedHeading: 'CPU:', expectedValue: 'CPU: 78 vCPU (+ 0.11 vCPU/worker)'}
]
...
clusterRecommendationTests.forEach(function (item) {
  test(clusterRecommendationSection + ': Check ' + item.name, async ({ page }) => {
    ...
    const value = await page.innerText('.row div:nth-child(1) .content > div .val');
...
const eicClientResourceRequirementsSection = 'EIC Client Resource Requirements:'
const clientResourceRequirementsSection = 'Client Resource Requirements:'
const clientResourceRequirementsTests = [
  {index: 1, name: 'CPU', expectedHeading: 'CPU:', expectedValue: '2 vCPU'},
  {index: 2, name: 'Memory', expectedHeading: 'Memory:', expectedValue: '4 GB'},
  {index: 3, name: 'Storage', expectedHeading: 'Storage:', expectedValue: '90 GB'}
]

clientResourceRequirementsTests.forEach(function (item) {
  test(eicClientResourceRequirementsSection + ': Check ' +  item.name, async ({ page }) => {
    await goToDeploymentSetup(page);
    await selectStandardSizeEICResourceOverview(page);
...
const clusterResourceOverviewTests = [
  ...
  {index: 2, name: 'Total ConfigMaps', expectedValue: 'Total ConfigMaps: 102'},
